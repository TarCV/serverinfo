 ...
 */
/**
* Changed by Const, so it can be used in SkullTag/Zandronum client
*/
/**
...
class Huffman {
	...
	public function setDictionary($dictionary) {
		...
		$this->root->setDictionary($dictionary,0, $this->leaves);
	...
	/**
	* Converts a 8-bit integer to a letter
	 *
	 * @param value The 8-bit integer.
	 * @return The 1-letter string.
	*/
	protected function encodeByte($byte) {
		return chr($this->reverseMap[$byte]);
	}
	/**
	 * Converts a 32-bit integer to a 4-letter string using some encoding
	 *
	 * @param value The 32-bit integer.
	 * @return The 4-letter string.
	 */
	protected function intToString($value) {
		return $this->encodeByte(($value >> 24) & 0xFF)
					.$this->encodeByte(($value >> 16) & 0xFF)
					.$this->encodeByte(($value >> 8) & 0xFF)
					.$this->encodeByte($value & 0xFF);
	}
	/**
	 * Converts a 32-bit integer to a 4-letter string without any encoding
	 *
	 * @param value The 32-bit integer.
	 * @return The 4-letter string.
	 */
	protected function intToString2($value) {
	 ...
	 */
	public function compressData($odata,$asArray = false) {
		$dword = 0;		// Current 32-bit $value.
		$bitsLeft = 32;	// Number of bits left in $dword.

		if(is_string($odata))
			$data = str_split($odata);
		else
			$data = $odata;
			
		// reserve place for padding signal.
		if(!$asArray)
			$compressed = ' ';
		else	{
			$compressed = array();
		...
		if(!$asArray) {
			$padBytes = floor($bitsLeft / 8);
			$bitsLeft = $bitsLeft % 8;
			$compressed .= substr($this->intToString($value), 0, 4-$padBytes);
			$compressed{0} = chr($bitsLeft & 0xFF);
			if (strlen($compressed)-1 > strlen($odata))
				return chr(0xff) . $odata;
		} else {
			$compressed[] = $value;
			$compressed[0] = $bitsLeft;
		}
		return $compressed;
	}
	...
	protected function stringToInt($str) {
		return ($this->reverseMap[ord(substr($str,0,1))] << 24)
				| ($this->reverseMap[ord(substr($str,1,1))] << 16)
				| ($this->reverseMap[ord(substr($str,2,1))] << 8)
				| $this->reverseMap[ord(substr($str,3,1))];
	}
	...
	public function decompressData($compressed,$asArray = false,$startIndex = 0) {
		...
		if(is_string($compressed)) {
			$padBits = ord($compressed{$compressedIndex});
			if (255 == $padBits)	return substr($compressed, $compressedIndex+1);
			$count = strlen($compressed) - 1 - $startIndex;
		} else {
			$padBits = $compressed[$compressedIndex];
			$count = count($compressed) - $startIndex;
		}
		$bitsLeft = ($count << 3) - (0xff & $padBits);
		$compressedIndex++;
...
class Node {
	 ...
	 */
	public function setDictionary(&$dictionary,$bitLength, &$leaves) {
		// Left branch.
		if($dictionary[$dictionary["nextIndex"] + 1] == $bitLength + 1) {
			...
			$this->zeroChild->bitLength = $bitLength+1;
			$this->zeroChild->bits = $dictionary[$dictionary["nextIndex"] + 2];
			$leaves[$dictionary[$dictionary["nextIndex"]]] = $this->zeroChild;
			$dictionary["nextIndex"] += 3;
		} else {
			$this->zeroChild = new Node();
			$this->zeroChild->myParent = $this;
			$this->zeroChild->setDictionary($dictionary,$bitLength + 1, $leaves);
		}
		...
		if($dictionary[$dictionary["nextIndex"] + 1] == $bitLength + 1) {
			...
			$this->oneChild->bitLength = $bitLength+1;
			$this->oneChild->bits = $dictionary[$dictionary["nextIndex"] + 2];
			$leaves[$dictionary[$dictionary["nextIndex"]]] = $this->oneChild;
			$dictionary["nextIndex"] += 3;
		} else {
			$this->oneChild = new Node();
			$this->oneChild->myParent = $this;
			$this->oneChild->setDictionary($dictionary,$bitLength + 1, $leaves);
		}
	}
